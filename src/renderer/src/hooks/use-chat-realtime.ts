...
export function useChatRealtime() {
    ...
    useEffect(() => {
        ...
        const fetchChats = async () => {
            try {
                ...
                const response = await fetchWithNeonColdBootRetry("https://web.yahla.org/api/chats", {
                    cache: "no-store",
                });
                if (!response.ok) {
        ...
        const fetchMessages = async () => {
            try {
                ...
                const response = await fetchWithNeonColdBootRetry(
                    `https://web.yahla.org/api/messages?chatRoomId=${encodeURIComponent(selectedChatId)}&limit=20`,
                    { cache: "no-store" }
                );
                if (!response.ok) {
    ...
    useEffect(() => {
        ...
        const connect = () => {
            ...
            socket = new WebSocket(`wss://web.yahla.org/api/realtime`);
            setSocket(socket);
