...
export default function SettingsSubsectionLastSeen() {
    ...
    const translations = {
        en: {
            title: 'Last seen',
            subtitle: 'Who can see my last seen',
            everyone: 'Everyone',
        ...
        ar: {
            title: 'آخر ظهور',
            subtitle: 'من يمكنه مشاهدة آخر ظهور',
            everyone: 'الجميع',
    ...
    const [selectedValue, setSelectedValue] = useState(session?.user?.whoCanSeeLastSeen || 'all');
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);

    // Sync with session when it loads
    useEffect(() => {
        if (session?.user?.whoCanSeeLastSeen) {
            setSelectedValue(session.user.whoCanSeeLastSeen);
        }
    }, [session]);
